<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-gradient-to-b from-cyan-400 to-blue-600 border-r border-blue-500 sm:translate-x-0 shadow-[4px_0_24px_rgba(0,0,0,0.1)]" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-transparent">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{ route('account.settings') }}" class="flex items-center p-2 rounded-lg group transition-all {{ request()->routeIs('account.settings') ? 'bg-white/20 backdrop-blur-sm shadow-sm border border-white/10 text-white font-semibold' : 'text-white/90 hover:bg-white/10 hover:text-white font-medium' }}">
               <svg class="flex-shrink-0 w-5 h-5 transition duration-75 {{ request()->routeIs('account.settings') ? 'text-white' : 'text-white/70 group-hover:text-white' }}" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
